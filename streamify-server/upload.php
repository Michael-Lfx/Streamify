<?php
	...
	preg_match_all('!\d+!', $_FILES['userfile']['name'], $matches);
	$content = "";
	$content = $content."\n#EXTINF:10.0";
	$content = $content."\nhttp://54.251.250.31/streams/".$username."/audio/".$_FILES['userfile']['name'];
	file_put_contents($user_folder.'/a.m3u8', $content, FILE_APPEND);
	unlink($audio_folder.'sound'.($matches[0][0] - 3).'.aac');
